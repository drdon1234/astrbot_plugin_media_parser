...
本文记录当前 30 个平台解析器的取数方式与边界，顺序与配置、工厂及本地发现列表一致。它描述代码实现，不将过去样本验证视为上游接口持续可用的保证；使用配置见 [README](../README.md)，公共数据流见 [架构文档](ARCHITECTURE.md)。
...
1. 先判内容形态再取字段。视频、图集、音频、动态、番剧、帖子、游戏页的数据结构不一样。
...
优先读取结构化页面状态，缺失时使用页面实际提供的内联媒体字段。短链 `v.kuaishou.com` 先跳转到真实页面；部分域名或路径还会被改写到更容易取到状态的移动页面。
...
页面未提供完整 SSR 状态时，当前实现会读取 `photoUrl`、`videoUrl`、`srcNoMark`、`window.rawData` 等候选字段。使用这些字段时仍需验证目标内容和媒体完整性，不能仅凭封面判定图集解析成功。
...
支持 `www.ximalaya.com`、`m.ximalaya.com` 与裸域名的 `/sound/{id}`、`/{数字主播ID}/sound/{id}`，按单集编号去重，保留消息中的提链顺序；输出规范 `/sound/{id}` 链接。专辑、评论详情、短链和应用私有协议不接入。

通过匿名 `https://m.ximalaya.com/tracks/{id}.json` 取得标题、主播、简介、封面、时长与 `play_path_64`、`play_path_32`、`play_path` 候选。验证单集身份，清洗 HTML，媒体只接受平台 CDN 域名。候选内出现 `_preview_` 时区分试听，不与普通音频混成同一组；付费或状态未知的可用 URL 不声明为完整音频，接口没有音频仍保留图文信息。

高赞评论使用移动接口 `/m-revision/common/track/queryTrackCommentsFirstPage`，按 `trackId` 验证归属，保留首屏顺序、去重并排除楼中楼，最多 20 条；不把页码增加当作已验证的分页，不用普通评论接口补足。关闭评论不发请求，评论失败保留音频与详情。所有 JSON 响应有体积限制、不跟随重定向，取消操作向上传播，不增加 Cookie、代理或新依赖。
...
取得微博状态 ID 后请求 `/ajax/statuses/buildComments`，复用访客 Cookie 与请求头。当前只读取一次返回列表，在取得的评论内按点赞数排序并限制条数；没有状态 ID 或评论失败时保留正文和媒体，不把结果视为全帖评论排名。
...
同一商品详情中的播放类 URL 在当前实现中组成一个视频候选组，不拆为多个独立视频项。
...
如果 `articleInfo` 里没有视频播放令牌就按普通图文微头条处理；存在 `playAuthTokenV2` 时沿用视频页的播放信息链路。
...
评论使用移动前端的 `https://api.toutiaoapi.com/article/v4/tab_comments/`，优先根据页面 `sessionConfig.groupId`，再用 `articleInfo.gid` 等字段确定 `group_id`，校验响应资源身份后分页读取，保留平台排序。文章、视频与微头条共用此评论链路。
...
视频字段由接口是否实际下发决定；当前视频提取逻辑按这些字段实现，既有验证记录尚未覆盖真实公开视频帖的完整下载与发送。
...
既有验证覆盖公众号普通文章与纯图集、视频号换票、HTTP 201 预览响应及媒体 CDN Range 访问；该记录不包含 AstrBot 内完整下载与发送验证。
...
首楼附件字段实际拼写为 `attches`，图片项使用 `type: "img"` 和 `attachurl`，仅补齐正文尚未出现的图片，保留原地址及查询参数；正文可仅有文字，也可包含一张或多张图片。

NGA 通过 `image_tls_ciphers` 指定 `ECDHE+AESGCM:ECDHE+CHACHA20`，处理已观察到的图片 CDN 握手拒绝。统一图片下载器在请求级别使用缓存的 SSLContext；保留默认 TLS 版本范围、证书和主机名校验，不改写共享会话。
...
通过匿名 `GET https://www.v2ex.com/api/topics/show.json?id={主题ID}` 读取公开主题，核对返回的主题身份后提取标题、作者、发布时间和主帖正文。图片仅来自主帖内容，不混入头像或回复配图；不读取网页中的后续附言。匿名接口不携带用户 Cookie 或 Token，登录限制、删除、限流和无效响应分别按失败处理，不用搜索摘要替代正文。

### 评论

评论数量复用 `message.hot_comments.count`，默认 `0`；`message.hot_comments.v2ex` 默认开启，关闭平台热评或选择 `仅富媒体` 时不发起评论请求。

匿名旧回复接口 `/api/replies/show.json?topic_id={主题ID}` 返回楼层顺序评论，没有感谢数，实测分页参数无效，因此只请求一次，不构造无效分页。开启评论后，另读取主题网页前 5 页（每页最多 100 楼）的 JSON-LD 感谢统计，按回复 ID、作者与时间对齐后，在已取得范围内按感谢数降序优先展示，其他评论仍按楼层顺序补齐。网页不可用或未发现获感谢的回复时使用匿名接口的普通顺序，不声称是官方热评或全帖最高热度。
...
仅接入 `/post/{文章ID}` 的公开文章，通过匿名 `POST https://api.juejin.cn/content_api/v1/article/detail` 读取 `article_info.web_html_content`。校验顶层及文章详情身份、公开状态后，提取标题、作者和发布时间；HTML 清洗保留代码缩进、列表、表格和链接。只把正文中的掘金图床图片交给下载器，保留完整签名查询参数。
...
仅接入 `blog.csdn.net/{作者}/article/details/{文章ID}`，从公开 HTML 的 `articleContentId`、`content_views` 及文章元数据读取标题、正文、作者、时间，核对规范链接。排除页面推荐、广告、隐藏内容和代码工具栏，保留代码缩进、段落及正文链接；图片仅使用正文内的 CSDN 图床地址。
...
仅接入 `www.cnblogs.com/{博客}/p/{文章ID}` 及 `.html` 形式，使用同一文章的规范路径处理有限重定向。正文限定为 `cnblogs_post_body`，标题来自 `cb_post_title_url`，时间来自可见 `post-date`，作者来自身份核验后的 `BlogPosting` 结构化数据。代码块保留缩进，图片仅来自正文中的博客园图床。
...
通过官方匿名 `GET https://gitee.com/api/v5/repos/{owner}/{repo}` 读取公开仓库概况，展示简短介绍、主要语言、Star/Fork 数、许可证及更新时间，不读取 README 或仓库代码。接口可在 HTTP 200 中返回迁移后的仓库身份，以一致的 `full_name`、`namespace`、规范地址为准；组织所有者不使用可能指向管理员个人的 `owner.login`。
...
仅接入 `gitlab.com`，项目路径支持多级子组。通过匿名 `GET /api/v4/projects/{编码后的完整路径}?license=true` 获取概况，再用项目数字 ID 获取语言统计；语言失败保留其余信息，缺失归档状态不推断为未归档。项目迁移仅接受经过主机、接口和项目身份校验的有限跳转。

Issue 接入 `/-/issues/{编号}` 及 Issue 类型的 `/-/work_items/{编号}`，读取 `/api/v4/projects/{项目ID}/issues/{编号}`，核对项目 ID、议题 ID、公开状态及工作项类型。讨论经匿名 `/api/graphql` 查询，按项目与议题全局 ID 校验，最多读取 3 页讨论，每个讨论最多 20 条记录，排除系统事件、去重并受总条数限制；使用普通顺序，不推断热门或点赞数。

GitLab 和 Gitee 均复用现有文本元数据和评论链路；仓库概况不请求评论，`仅富媒体` 没有可发送媒体。稀土掘金、CSDN、博客园、Gitee 和 GitLab 均无需额外依赖、Token、Cookie 或代理配置。评论全局条数默认 `0`，支持评论的平台开关默认开启；只有输出模式包含文本、全局条数大于零且对应开关开启时才读取评论，博客园不接入评论。响应有界，取消操作继续向上传播。
