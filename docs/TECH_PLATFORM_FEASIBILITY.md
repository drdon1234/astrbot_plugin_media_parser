...
调研日期：2026-09-24（北京时间）。以下保留接入前的实测依据；随后已按公开文章、仓库概况及 Issue 的范围接入五个平台，当前实现与边界见 [平台解析备忘](PARSER_METHOD_MEMO.md)。

## 结论

| 平台 | 正文或仓库概况 | 评论实际结果 | 建议 |
| --- | --- | --- | --- |
| 稀土掘金 | 两篇公开文章的详情 JSON 和网页正文均成功，另验证一篇正文图片 | 两篇分别取得 3 条一级评论、2 条一级评论及 1 条作者回复；游标分页成功 | 可接入公开文章图文与评论，维护成本中等 |
| CSDN | 免费博客正文、代码块和图片成功；VIP 样本只有公开预览 | 免费样本两页各 10 条根评论，另含回复；另一作者样本取得 7 条评论 | 可接入公开文章及评论，明确区分全文和付费预览 |
| 博客园 | 两篇不同作者博客的标题、作者、时间和正文成功，正文图片可访问 | 两篇有 4/10 条评论，但接口只返回数量，明确要求登录查看 | 可接入公开博客正文，暂不能承诺匿名评论 |
| Gitee | 两个公开仓库概况成功，包括迁移后的规范仓库信息 | Issue 评论两页各 5 条，均含正文、作者和时间，ID 不重复 | 可按 GitHub 模式接入仓库概况；Issue 评论可单独支持 |
| GitLab | 两个普通项目和一个多级子组项目的公开概况成功；语言、许可证也已取得 | REST Notes 返回 401；官方 GraphQL 匿名取得两页讨论，共 12 条非系统记录 | 可按 GitHub 模式接入仓库概况；若支持 Issue 链接，讨论也有可用路径 |
...
支持范围先限定公开 `/post/<id>` 技术文章；旧 `juejin.im` 同 ID 跳转已验证，真实分享短链、沸点、付费小册未验证。
...
建议先支持 `blog.csdn.net/{user}/article/details/{id}`。主要维护成本来自正文 DOM、网站内部评论接口和 WAF；本次未触发挑战。付费或受限文章只能按可见预览说明，不能当完整正文发送。
...
建议先做公开博客图文解析，评论登录限制不影响正文。样本为 `/{blog}/p/{id}`；接入时已补测 `.html`，可规范化到同一文章。历史日期路径和自定义域名未接入。
...
1. 按资讯内容价值优先：掘金、CSDN 的公开图文＋有限评论已具备实测依据；博客园先支持正文。
2. 按实现成本优先：Gitee、GitLab 的仓库概况可沿用 GitHub 的输出方式，但各自持有本平台实现；若纳入 Issue 链接，再附加已验证的对应讨论。
3. 复用 `MediaMetadata`、现有下载与文本渲染、评论数量和输出模式控制，无需新建跨平台共享层。代码块和文章内图片清洗是技术资讯平台的主要实现工作。
4. 评论应按来源语义展示热门或普通评论，不承诺本次未验证的最热排序；获取失败不应丢失正文。当前出口均存在可用直连路径，是否需要代理需以部署环境实际请求为依据。

## 本地证据

原始响应、抽取结果和调研脚本均保存在被忽略的 `test/` 下，不纳入 Git：
