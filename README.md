...
`消息与展示 → 媒体展示 → 重复链接不再发送任何内容` 默认关闭。开启后，同一标准链接成功发送过一次后，再次解析该链接会整条静默跳过，不发送文本、热评、图片、视频或翻译。记录会持久化到插件运行时目录，短链解析完成后的最终链接也会作为别名识别；记录有效期与 `媒体中转 → 中转缓存有效期(秒)` 一致，过期后同一链接可重新发送。
