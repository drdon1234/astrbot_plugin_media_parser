...
1. **依赖库**：插件安装时会依据 `requirements.txt` 安装 `aiohttp>=3.9`、`cryptography` 和本地二维码生成依赖 `qrcode[pil]`
...
`消息输出 → 富媒体展示 → 视频仅发送封面` 开启后，插件不会发送视频节点，会把每个视频改为图片节点发送：解析结果自带封面时直接使用封面；没有封面时会通过本地流式中继交给 ffmpeg 截取视频第一帧。无封面截帧依赖缓存目录可用且运行环境存在 ffmpeg，远端视频输入仍受下载大小硬限制，超限或截帧失败时该封面会被跳过。
...
- **图片处理**：非 JPG/PNG 图片会尝试用 ffmpeg 转换为 PNG；缺少 ffmpeg 时保留原格式并在解析信息中记录警告
