...
也可以直接发送 `主动更新 Cookie 指令`（默认 `B站更新Cookie`），立即发起扫码更新，不受自动请求冷却限制。该指令仅管理员可用：管理员身份同时识别 AstrBot 全局管理员（`admins_id`）与插件 `权限控制 → 管理员 ID`，私聊与群聊中均可触发；非管理员发送或两种管理员均未配置时会收到明确提示。指令留空即可关闭主动入口。
